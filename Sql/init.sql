...
COMMENT ON type sys_user_roles IS '用户角色（ADMIN=管理员，USER=普通用户）';
...
COMMENT ON type ledger_book_type IS '账本类型（个人/家庭/办公/共享/其他）';
...
COMMENT ON type ledger_record_type IS '交易类型（收入/支出/转账）';
...
COMMENT ON type ledger_account_attribute IS '账户属性（普通/信用卡）';
...
-- ===================== 函数 =====================
-- 创建自动创建分区的函数（按月分区）
CREATE OR REPLACE FUNCTION auto_create_ledger_partition()
    RETURNS TRIGGER
AS
$$
DECLARE
...
BEGIN
    ...
    partition_name := 'ledger_account_records_' || TO_CHAR(NEW.occurred_at, 'YYYYMM');
    -- 计算分区的时间范围（带时区，匹配occurred_at类型）
    partition_start := DATE_TRUNC('month', NEW.occurred_at)::timestamptz;
    partition_end := DATE_TRUNC('month', NEW.occurred_at) + INTERVAL '1 month';
    -- 检查分区是否存在，不存在则创建
...
CREATE INDEX idx_password_entry_user_created ON cipher_password_entry (user_id, created_at)
    WHERE
        deleted = FALSE;
...
-- 账本表索引
CREATE UNIQUE INDEX idx_ledger_books_user_book_name_active ON ledger_books (user_id, book_name)
    WHERE
        deleted = FALSE;
...
COMMENT ON COLUMN sys_users.user_id IS '用户ID（UUID）';

COMMENT ON COLUMN sys_users.username IS '用户名（唯一）';

COMMENT ON COLUMN sys_users.email IS '邮箱（唯一）';

COMMENT ON COLUMN sys_users.password_hash IS '密码哈希值';

COMMENT ON COLUMN sys_users.roles IS '用户角色';

COMMENT ON COLUMN sys_users.created_at IS '创建时间';

COMMENT ON COLUMN sys_users.updated_at IS '更新时间';

COMMENT ON COLUMN sys_users.is_active IS '是否激活（true=是，false=否）';

COMMENT ON COLUMN sys_users.last_login IS '上次登录时间';

COMMENT ON COLUMN sys_users.deleted IS '逻辑删除（false=未删除，true=已删除）';

COMMENT ON COLUMN sys_users.deleted_at IS '逻辑删除时间';

-- 密码信息表
COMMENT ON TABLE cipher_password_info IS '密码管理信息表（存储主密码相关）';

COMMENT ON COLUMN cipher_password_info.info_id IS '信息ID（UUID）';

COMMENT ON COLUMN cipher_password_info.user_id IS '关联用户ID';

COMMENT ON COLUMN cipher_password_info.master_verify_hash IS '主密码验证哈希';

COMMENT ON COLUMN cipher_password_info.verify_salt IS '盐值（用于主密码加密）';

COMMENT ON COLUMN cipher_password_info.version IS '密码版本号';

COMMENT ON COLUMN cipher_password_info.created_at IS '创建时间';

COMMENT ON COLUMN cipher_password_info.updated_at IS '更新时间';

COMMENT ON COLUMN cipher_password_info.deleted IS '逻辑删除（false=未删除，true=已删除）';

COMMENT ON COLUMN cipher_password_info.deleted_at IS '逻辑删除时间';

-- 密码条目表
COMMENT ON TABLE cipher_password_entry IS '密码条目表（存储各平台账号密码）';

COMMENT ON COLUMN cipher_password_entry.password_id IS '密码条目ID（UUID）';

COMMENT ON COLUMN cipher_password_entry.user_id IS '关联用户ID';

COMMENT ON COLUMN cipher_password_entry.website IS '网站/APP名称';

COMMENT ON COLUMN cipher_password_entry.account IS '平台账号';

COMMENT ON COLUMN cipher_password_entry.email IS '关联邮箱';

COMMENT ON COLUMN cipher_password_entry.password IS '平台密码（加密存储）';

COMMENT ON COLUMN cipher_password_entry.url IS '网站地址';

COMMENT ON COLUMN cipher_password_entry.totp_token IS 'TOTP令牌';

COMMENT ON COLUMN cipher_password_entry.recovery_code IS '恢复码';

COMMENT ON COLUMN cipher_password_entry.notes IS '备注信息';

COMMENT ON COLUMN cipher_password_entry.created_at IS '创建时间';

COMMENT ON COLUMN cipher_password_entry.updated_at IS '更新时间';

COMMENT ON COLUMN cipher_password_entry.deleted IS '逻辑删除（false=未删除，true=已删除）';

COMMENT ON COLUMN cipher_password_entry.deleted_at IS '逻辑删除时间';

-- 账本表
COMMENT ON TABLE ledger_books IS '账本表，支持个人/家庭/办公/共享等多种类型，多币种配置';

COMMENT ON COLUMN ledger_books.book_id IS '账本唯一标识，UUID格式，主键';

COMMENT ON COLUMN ledger_books.user_id IS '所属用户ID，关联sys_users表user_id字段，不可为NULL';

COMMENT ON COLUMN ledger_books.book_name IS '账本名称，用户自定义，同一用户下未删除的账本名称不可重复';

COMMENT ON COLUMN ledger_books.book_type IS '账本类型，枚举类型：PERSONAL(个人)、FAMILY(家庭)、OFFICE(办公)、SHARE(共享)、OTHER(其他)，默认PERSONAL';

COMMENT ON COLUMN ledger_books.currency IS '账本币种，遵循ISO 4217标准：CNY(人民币)、USD(美元)、EUR(欧元)等，默认CNY';

COMMENT ON COLUMN ledger_books.description IS '账本描述信息，用户自定义，说明账本用途，默认空字符串';

COMMENT ON COLUMN ledger_books.icon IS '账本展示图标，前端图标类名：如icon-book(书本)、icon-wallet(钱包)，默认icon-book';

COMMENT ON COLUMN ledger_books.status IS '账本状态，true=启用（可新增交易），false=禁用（仅可查看），默认true';

COMMENT ON COLUMN ledger_books.created_at IS '账本创建时间，自动填充当前时间，带时区';

COMMENT ON COLUMN ledger_books.updated_at IS '账本最后更新时间，带时区，通过触发器自动刷新';

COMMENT ON COLUMN ledger_books.deleted IS '软删除标记，true=已删除，false=正常使用，默认false';

COMMENT ON COLUMN ledger_books.deleted_at IS '软删除时间，仅当deleted=true时有值，带时区，通过触发器自动设置';

-- 账户表
COMMENT ON TABLE ledger_account IS '账本下的账户表，记录各类账户信息与余额，支持普通账户与信用账户';

COMMENT ON COLUMN ledger_account.account_id IS '账户唯一标识，UUID格式，主键';

COMMENT ON COLUMN ledger_account.book_id IS '所属账本ID，关联ledger_books表book_id字段，不可为NULL';

COMMENT ON COLUMN ledger_account.account_name IS '账户名称，用户自定义：如"招商银行储蓄卡"、"招行信用卡"，不可为NULL';

COMMENT ON COLUMN ledger_account.account_type IS '账户类型，字符串标识：CASH(现金)、BANK(银行卡)、CREDIT_CARD(信用卡)、INVESTMENT(投资)等';

COMMENT ON COLUMN ledger_account.account_attribute IS '账户属性，枚举类型：NORMAL(普通账户，余额不可为负)、CREDIT(信用账户，余额可负)，默认NORMAL';

COMMENT ON COLUMN ledger_account.account_balance IS '账户余额，数值类型保留2位小数，普通账户≥0，信用账户可负，默认0.00';

COMMENT ON COLUMN ledger_account.currency IS '账户币种，遵循ISO 4217标准，需与所属账本币种一致，默认CNY';

COMMENT ON COLUMN ledger_account.icon IS '账户展示图标，前端图标类名：如icon-wallet(钱包)、icon-card(银行卡)，默认icon-wallet';

COMMENT ON COLUMN ledger_account.sort_order IS '排序值，整数类型，数值越小前端展示越靠前，默认0';

COMMENT ON COLUMN ledger_account.created_at IS '账户创建时间，自动填充当前时间，带时区';

COMMENT ON COLUMN ledger_account.updated_at IS '账户最后更新时间，带时区，通过触发器自动刷新（含余额更新）';

COMMENT ON COLUMN ledger_account.deleted IS '软删除标记，true=已删除，false=正常使用，默认false';

COMMENT ON COLUMN ledger_account.deleted_at IS '软删除时间，仅当deleted=true时有值，带时区，通过触发器自动设置';

-- 收支类型表
COMMENT ON TABLE ledger_expense_income_types IS '收入支出类型表（基础分类）';

COMMENT ON COLUMN ledger_expense_income_types.types_id IS '类型ID（UUID）';

COMMENT ON COLUMN ledger_expense_income_types.type_name IS '类型名称（如餐饮、工资）';

COMMENT ON COLUMN ledger_expense_income_types.type_category IS '类型类别（EXPENSE=支出，INCOME=收入）';

COMMENT ON COLUMN ledger_expense_income_types.icon IS '图标名称';

COMMENT ON COLUMN ledger_expense_income_types.created_at IS '创建时间';

COMMENT ON COLUMN ledger_expense_income_types.updated_at IS '更新时间';

COMMENT ON COLUMN ledger_expense_income_types.deleted IS '逻辑删除（false=未删除，true=已删除）';

COMMENT ON COLUMN ledger_expense_income_types.deleted_at IS '逻辑删除时间';

-- 账户交易记录表
COMMENT ON TABLE ledger_account_records IS '账户交易记录表，记录收入、支出、转账等交易行为，按时间分区存储';

COMMENT ON COLUMN ledger_account_records.record_id IS '交易记录唯一标识，UUID格式，主键';

COMMENT ON COLUMN ledger_account_records.account_id IS '所属账户ID，关联ledger_account表account_id字段，不可为NULL';

COMMENT ON COLUMN ledger_account_records.book_id IS '所属账本ID，关联ledger_books表book_id字段，冗余存储提升查询效率，不可为NULL';

COMMENT ON COLUMN ledger_account_records.amount IS '交易金额，数值类型保留2位小数：收入=正数，支出=负数，转账需关联两个账户生成等额正负记录，不可为0';

COMMENT ON COLUMN ledger_account_records.record_type IS '交易类型，枚举类型：INCOME(收入)、EXPENSE(支出)、TRANSFER(转账)，不可为NULL';

COMMENT ON COLUMN ledger_account_records.type_id IS '关联收支类型表的ID，不可为NULL';

COMMENT ON COLUMN ledger_account_records.type_name IS '交易分类名称，用户自定义：如"餐饮美食"、"工资收入"、"房租支出"，默认空字符串';

COMMENT ON COLUMN ledger_account_records.description IS '交易备注信息，用户自定义补充说明，默认空字符串';

COMMENT ON COLUMN ledger_account_records.occurred_at IS '交易发生时间，可自定义（如补录历史交易），带时区，用于分区表拆分，不可为NULL';

COMMENT ON COLUMN ledger_account_records.created_at IS '交易记录创建时间，自动填充当前时间，带时区，不可手动修改';

COMMENT ON COLUMN ledger_account_records.updated_at IS '交易记录最后更新时间，带时区，通过触发器自动刷新';

COMMENT ON COLUMN ledger_account_records.deleted IS '软删除标记，true=已删除，false=正常使用，默认false';

COMMENT ON COLUMN ledger_account_records.deleted_at IS '软删除时间，仅当deleted=true时有值，带时区，通过触发器自动设置';
